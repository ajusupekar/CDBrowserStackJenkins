...
Feature: To test registration functionality:  
  ...
  @PAD-3148_TC05_AndroidTorFx  @regression
  ...
  @PAD-3148_TC13_AndroidTorFx @regression
  ...
  @PAD-3430_TC14_AndroidTorFx @regression
  ...
  Scenario Outline: 6.2 To verify when user submits the correct OTP then user navigates to the create Password screen
    ...
    Examples: 
     ...
     | John      | Sweven   | TorFxRegisterationUserA |    01012000 | United Kingdom |     123 | +91         | India           |   9156379129 | UAT         |

  # Error message : Not defined
  @PAD-3152_TC53_AndroidTorFx @regression
  Scenario Outline: To verify when user submits the wrong OTP then system should generate error message to provide correct OTP
    ...
    When android user click on continue button on mobile screen
    Then android user landed on OTP screen
    Then android user enters invalid OTP "<invalidOTP>"
    Then android app should generate error message to provide correct OTP

    Examples: 
    | FirstName | LastName | EmailID                | DateOfBirth | country        | address | countryCode | countrySelected | mobileNumber | environment | invalidOTP        |
    | John      | Sweven   | TorFxRegisterationUser |    01012000 | United Kingdom |     123 | +91         | India           |   9156379129 | UAT         | AndroidInvlaidOTP |

  @PAD-3152_TC54_AndroidTorFx @regression
  ...
  #| John      | Sweven   | TorFxRegisterationUser  |    01012000 | India   | 123     |  91563791291  |
  ...
  Scenario Outline: 7.1 To verify that when user is on set Password screen and presses device back button then the user should remain on create Password screen
    ...
    Examples: 
    | FirstName | LastName | EmailID                 | DateOfBirth | country | address | mobileNumber | environment |
    | John      | Sweven   | TorFxRegisterationUserB |    01012000 | India   |     129 |   8668272780 | UAT         |
  ...
  Scenario Outline: 7.2 To verify the all parameter's of set Password screen when user landed on set password screen
    ...
    Examples: 
    | FirstName | LastName | EmailID                 | DateOfBirth | country | address | mobileNumber | environment |
     | John      | Sweven   | TorFxRegisterationUserC |    01012000 | India   |     129 |   8668272780 | UAT         |
  ...
  Scenario Outline: 7.5 To verify that user selects the hide/show button then the characters entered into the password field should be masked/visible respectively
    ...
    Examples: 
    | FirstName | LastName | EmailID                | EmailID           | DateOfBirth | country | address | mobileNumber | environment | password    |
    | John      | Sweven   | TorFxRegisterationUserD| TorFxRegisterationUser |    01012000 | India   |     129 |   8668272780 | UAT         | Password@12 |
  ...
  @PAD-3153_TC59_AndroidTorFx 
   ...
   Scenario Outline: 7.6,7.7 To verify that user selects the T&Cs acknowledgment switch then T&C's should be marked as 'acknowledged' by the user respectively
    ...
    Examples: 
    | FirstName | LastName | EmailID                | EmailID           | DateOfBirth | country | address | mobileNumber | environment | password    |
    | John      | Sweven   | TorFxRegisterationUserE| TorFxRegisterationUser |    01012000 | India   |     129 |   8668272780 | UAT         | Password@12 |
  ...
  Scenario Outline: 7.11,7.12 To verify that continue button when user selects the T&Cs acknowledgment switch
    ...
    When android user click on continue button on mobile screen
    Then android user landed on OTP screen
    When android user enters valid otp "<EmailID>" 
    Then android user landed on set password screen
    And android user entered password "<password>"
    Then android user selects T&Cs acknowledgment switch
    ...
    Examples: 
    | FirstName | LastName | EmailID                 | DateOfBirth | country | address | mobileNumber | environment | password    |
    | John      | Sweven   | TorFxRegisterationUserF |    01012000 | India   |     129 |   8668272780 | UAT         | Password@12 |

  @PAD-3153_TC61_AndroidTorFx @TorFxSanity @regression 
  Scenario Outline: To verify that continue button remains disabled when user do not enters Password or T&Cs
    ...
    Examples: 
    | FirstName  | LastName  | emailAddress            | DateOfBirth | country | address | mobileNumber | environment |
    | John1      | Sweven1   | TorFxRegisterationUserG |    01012000 | India   |     129 |   8668272780 | UAT         |
    ...
    Examples:
    | FirstName | LastName | EmailID             | DateOfBirth | country   | address | NIRC      | countryCode| countrySelected | mobileNumber |password   |
    | John      | Sweven   | RegisterationUser02 |    01012000 | Singapore |     148 | 789456485 |  +91       | India           |   8668272780   |Password@12|
    ...
    Examples:
    | FirstName | LastName | EmailID             | DateOfBirth | country   | address | countryCode| countrySelected | mobileNumber |password   |
    | John      | Rick     | RegisterationUser03 |    01012000 | South Africa|  345  |  +91       | India           |   8668272780   |Password@12|     
    | John      | Rick     | RegisterationUser04 |    01012000 | China     |    ab   |  +91       | India           |   8668272780   |Password@12|
    | John      | Rick     | RegisterationUser05 |    01012000 | United Kingdom|22   |  +91       | India           |   8668272780   |Password@12|
    | John      | Sweven   | RegisterationUser06 |    01012000 | France    |   148   |  +91       | India           |   8668272780   |Password@12|
    ...
    Examples:
    | FirstName | LastName | EmailID             | DateOfBirth | country   | address | NIRC      | countryCode| countrySelected | mobileNumber |password   |
    | Kelvin01  | Jackson  | RegisterationUser08 |    01012000 | Spain     |     148 | 789456485 |  +91       | India           |   8668272780 |Password@12|
  ...
  user then save button should be enabled
    ...
    Then android user clicks on continue button to set password
    Examples: 
    | FirstName | LastName | EmailID                | DateOfBirth | country        | address   |postcode  |state |countrySelected | mobileNumber |password   |
    | John      | Sweven   | TorFxRegisterationUser |    01012000 | New Zealand    | 129       |123       |abc   |India           |   8668272780 |Password@12|
    ...
    When android user click on continue button
    Then android user landed on Current address screen
     And android varifies USA and Canada country should not be avaiable in Country dropdown
